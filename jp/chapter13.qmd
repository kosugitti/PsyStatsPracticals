...
そこで，直線関数を0と1の範囲に入るように変形してやれば良い。変形には，次のロジスティック関数が用いられる。

$$ f(x) = \frac{1}{1 + e^{-x}} $$

ここで$x$を線形モデルでデザインし，線形モデルの予測値$p$をロジスティック関数にかけることで，0から1の範囲に収まる確率が得られる。
...
もともと線形モデルをロジスティック関数によって次のように変換したのであった。

$$ p = \frac{1}{1 + e^{-(\beta_0 + \beta_1 x)}} $$

この式を$x$について解いてみよう。

$$ p(1 + e^{-(\beta_0 + \beta_1 x)}) = 1 $$
$$ p + pe^{-(\beta_0 + \beta_1 x)} = 1 $$

$$ pe^{-(\beta_0 + \beta_1 x)} = 1 - p $$

$$ e^{-(\beta_0 + \beta_1 x)} = \frac{1-p}{p} $$

$$ -(\beta_0 + \beta_1 x) = \log \frac{1-p}{p} $$
...
このような正の整数しかとらないデータに対してはポアソン分布で回帰した方がよく，ポアソン分布の位置パラメータ$\lambda$は正の実数しかとらないため，指数変換することで対応する。すなわち，
$$ \lambda_i = \exp(b_0 + b_1 x_i) $$
として
