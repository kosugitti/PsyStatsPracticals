...
We have covered many statistical models. Step 2 of Bayesian-modeling learning is to rewrite those analyses in Stan.
...
The goal of statistics is to turn natural human behavior and responses into numbers and read meaning out of them; budget or environment should not be allowed to bend the behavior itself. Keep the data as raw as possible, and *think* about how to analyze them. Approach the question from "what mechanism generated this data?" The "mechanism" can equally be called a probability distribution. A single response cannot be deterministic; the possible alternative values, together, form a probability distribution. The parameters of that distribution are then specified by a mathematical structure.
...
Bayesian modeling answers all of these. Even data from a factorial design can be examined with new angles and new hypotheses.

Bayesian modeling is not limited to factor-effect identification under designed experiments. For data that are not normal, or that do not partition into discrete groups, you can build models from the data-generating-mechanism perspective. The examples below should convey how the analytic viewpoint changes. Statistical analysis is not the constrained application of pre-existing analyses to handed-over data; it is the creative work of imagining the data-generating mechanism.

### Step 4: know the limitations

Once the freedom and creativity of Bayesian modeling sink in, you may hit its limits. Enjoy the modeling first, but here is a preview.

The first issue is **model evaluation**. NHST gives a clear criterion: "if $p$ is below the chosen $\alpha$, declare a difference." Bayesian modeling does not yield a clean "yes or no" of that kind. How do we compare a null-hypothesis model against an alternative, or our novel model against an existing one?
...
Assuming model evaluation is handled, the next stumbling block is "I cannot design my own model." The models below are appealing, but designing them is intimidating. There is no shortcut, but "build the entire model from scratch" is an unnecessarily large ambition. Familiarize yourself with many models first, and imagine how each might fit your data. Or start with a tiny **toy model** and grow it incrementally — @Hamada201812 and @Hamada2020 make this point well.

Often a plain linear model is sufficient. After all this talk of customization, that may sound contradictory, but if the trend is clear, a linear model will mostly do. Linear models are not perfect but are widely understandable; modeling refines the fine details, and finer detail is often unnecessary in practice. "Linear" need not mean "simple regression": GLMs, mixed models, and various other extensions cover much ground.

Even with these limitations in mind, Bayesian modeling is well worth learning. To dismiss statistics without ever experiencing this free and creative world is a waste. The examples that follow will give a taste of what is possible.
...
Normal-distribution models in psychology almost always concern means via general linear models. The normal is parameterized by a location $\mu$ and a scale $\sigma$;[^4] the scale captures spread or measurement error. Let us model that.

[^4]: Some texts parameterize by the variance $\sigma^2$; we use the standard deviation $\sigma$ to match Stan's convention.
...
The estimated $\mu$ is near 150 — consistent with the manual. The per-individual errors are the point of interest; visualize them:

```{r gyudon10_visual}
pacman::p_load(bayesplot)
draws <- fit$draws()
# visualize sigma only
...
The recovered correlation is `r round(fit$summary("rho")$mean, 4)` — essentially the true value. Because each margin of a bivariate normal is itself a (univariate) normal, adding the $X_1$ information to the likelihood lets us use all the data we have. Even when observed data are limited, modeling the generating mechanism can recover the missing information. Truncated or censored data — say, scores capped at an upper or lower bound — can be similarly modeled to estimate what the "uncensored" value would have been.[^6]

[^6]: See, e.g., @kinosada2018.

## Models with non-normal distributions

Bayesian modeling sets a mathematical structure on the parameters of a probability distribution. Here are some non-normal examples.
...
$$ p_{ij} = \frac{1}{1 + \exp\bigl(-a_j (\theta_i - b_j)\bigr)}. $$
...
`exametrika` and most IRT software integrate out the (numerous) person parameters and use EM for efficient analytic estimation. MCMC explores the entire joint parameter space and so is much slower, but writing the Stan code makes the underlying computation explicit and gives you the freedom to customize.
...
[^10]: The denominator of Bayes's theorem, $P(D)$, is a normalizing constant unnecessary for the *shape* of the posterior. Sampling notation drops it for speed; `target` notation does not, so it is needed when the marginal likelihood matters.
...
`tp` is obtained by applying `softmax` to `lp`. Recall that `lp` is a vector of log-posteriors; softmax turns it into normalized probabilities:
...
Mathematical formalization can feel abstract (and harder), but the abstraction lets you recognize problems that are structurally the same. Approaching a problem from "what is the data-generating mechanism?" or "what probability model fits?" is a creative act.
...
A single residual SD `sig` is used. The prior on $\tau$ is non-informative; the intercepts `beta0a` and `beta0b` are centered near 80 kg; the slopes `beta1a` and `beta1b` are wide enough to permit either sign.
...
### Modeling a heterogeneous group
...
A word about the Poisson: its only parameter, $\lambda$, is the mean — and also the variance, by construction. When the data are over-dispersed (variance greater than the mean), a single-$\lambda$ Poisson fits poorly. The **negative binomial** distribution generalizes the Poisson with a separate dispersion parameter:
...
This parameterization is hard to read. Stan supplies the alternative `neg_binomial_2`, parameterized by the mean $\mu$ and a dispersion $\phi$:
...
As @Lee201709 puts it, **the limit of Bayesian modeling is the user's imagination.** Imagine many data-generating mechanisms and build interesting models.
