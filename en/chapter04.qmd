...
To emphasize text in place, use one asterisk for *italics* or two for **boldface**.
...
Figures are inserted as links to image files. The bracketed text is the caption; the parenthesized text is the path to the image:
...
For reproducibility, figures should also be specified by code. **Always visualize your data first.** Visualization reveals patterns and relationships that summary statistics alone can hide. After collecting data, the first step should be visualization — and we emphasize this because it bears emphasizing. For an extended treatment, drawing on examples from psychology, see @Kieran2018.
...
A *geometric object* (or **geom**) specifies how data should be visualized. `ggplot2` provides many varieties; a sampling:
...
- **`geom_boxplot()`** — box-and-whisker plots summarizing a distribution (median, quartiles, outliers).
...
We have introduced intermediate objects `g`, `g1`, `g2` to emphasize the layering; in practice, the whole chain is usually written as a single expression. Here both geoms share the same data and mapping; when several geoms share their data and aesthetics, it is cleaner to declare them at the canvas level:
...
For a report or paper, you may need monochrome figures. `ggplot2` chooses a color scheme automatically by selecting a default **palette**; changing the palette changes the colors used. For grayscale output, use the `Grays` palette.
...
p2 <- ggplot(mtcars, aes(x = wt, y = mpg, color = factor(cyl))) +
  ...
  ggtitle("Color-blind-friendly palette")
...
By default, `ggplot2` figures have a gray background, set by the default theme `theme_gray()`. The figure examples in the Japanese Psychological Association's [editorial guidelines](https://psych.or.jp/manual/) use a white background; for that look, switch to `theme_classic()` or `theme_bw()`.
...
4. (Continued.) Color the points by blood type. Use the `Set3` palette.
5. (Continued.) Use the point shape (not just color) to encode blood type.
...
# scatter plot colored by blood type
